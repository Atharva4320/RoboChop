...
#TODO: VISION LOOP
def vision_loop(img_queue, verts_queue, mask_queue, messenger):
	W = 848
	H = 480
	...
	while True :
		...
		obs_message, message_name = messenger.receive_object()
		# print(obs_message)
		if message_name == "Segment":
...
#TODO: SAM LOOP
def SAM_loop(img_queue, verts_queue, mask_queue, target_list, messenger, YOLO, SAM):
	 ...
	 while True:
			...
			### FIX ->      ### THIS NEEDS TO BE DONE PROPER###
				...
				messenger.send_object(msg_dict, "SAM_Message_Dictionary")
...
if __name__ == '__main__':
	...
	target_list, target_list_name = messenger.receive_object()
	# while True:
	...
	vision = threading.Thread(target=vision_loop, args=(img_queue, verts_queue, mask_queue, messenger))
	mask_sam = threading.Thread(target=SAM_loop, args=(img_queue, verts_queue, mask_queue, target_list, messenger, YOLO, SAM))
